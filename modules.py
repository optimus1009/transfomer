...
def multihead_attention(queries, keys, values,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE): # 因为每一轮的 映射matrix 都不一样。所以需要用不同的变量域，保证每个 W 都是不一样的
        # Linear projections
...
def positional_encoding(inputs,
    ...
    E = inputs.get_shape().as_list()[-1] # shape: [batch_size, time_step, embedding_size]
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # E: embedding_size
        position_enc = np.array([[pos / np.power(10000, (i-i%2)/E) for i in range(E)]for pos in range(maxlen)]) # (maxlen, E)
        ...
        # lookup
        outputs = tf.nn.embedding_lookup(position_enc, position_ind) # (N,T,E)
