...
def calc_num_batches(total_num, batch_size):
    ...
    return total_num // batch_size + int(total_num % batch_size != 0) # 如果除不尽，最后剩余的样本也应该加进去
...
def convert_idx_to_token_tensor(inputs, idx2token):
    ...
    return tf.py_func(my_func, [inputs], tf.string) # tf.py_func: Wraps a python function and uses it as a TensorFlow op
